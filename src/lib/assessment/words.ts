 ...
 * words (shuffled, unlabeled); the tribe mapping is backend scoring logic and is
 * never shown.
 *
 * Each word maps to one or more tribes by `slug`. A word shared across N tribes
 * splits a single point evenly — 1/N to each (see `score.ts`). For the common
 * two-tribe case this is the 0.5-each split described in the design doc and
 * ADR-0001; one word ("Zealous") maps to three tribes and so contributes 1/3 to
 * each. The 1/N rule conserves exactly one point per word regardless of how many
 * tribes it maps to, which keeps the coverage-fair normalization in `score.ts`
 * exact.
 *
 * NOTE on the count: `ASSESSMENT_DESIGN.md` heads the list "Total: 73 words",
 * but both the flat list and the mapping table enumerate the same 74 unique
 * words ("73" is a stale summary in the prose). We transcribe the enumerated
 * data faithfully (74) and derive `WORD_COUNT` from it rather than hard-coding
 * the stale number.
 */
