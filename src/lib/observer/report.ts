...
import { rankScores, type RankedTribe } from "@/lib/assessment/ranking";
import { aggregateObservers, hasEnoughObservers, MIN_OBSERVERS } from "./aggregate";
...
export interface ProfileBars {
  ...
  label: string;
  /**
   * Bar-fill fraction (0–1, relative to this profile's own top tribe) keyed by
   * tribe slug. A missing slug means that tribe scored zero for this profile.
   */
...
export interface UnlockedReport {
  ...
  /** Each Observer individually, anonymous, in submission order. */
  perObserver: ProfileBars[];
}

export type ComparisonReport = LockedReport | UnlockedReport;

/** Turn a ranked profile into a slug→relative lookup for the client bars. */
function relativeBySlug(ranked: RankedTribe[]): Record<string, number> {
  const map: Record<string, number> = {};
  for (const tribe of ranked) map[tribe.slug] = tribe.relative;
  return map;
}
 ...
 * it all into client-ready bar fractions ordered by the Subject's own ranking so
...
export async function buildComparisonReport(
  ...
  const selfRanked = rankScores(score(selfWords));
  const { averaged, perObserver } = aggregateObservers(responses);

  const order: ReportTribe[] = selfRanked.map((tribe) => ({
    slug: tribe.slug,
    name: tribe.name,
  ...
  return {
    ...
    order,
    self: { label: "You", relativeBySlug: relativeBySlug(selfRanked) },
    aggregate: {
      label: "All observers",
      relativeBySlug: relativeBySlug(rankScores(averaged)),
    },
    perObserver: perObserver.map((table, i) => ({
      label: `Observer ${i + 1}`,
      relativeBySlug: relativeBySlug(rankScores(table)),
    })),
  };
}
