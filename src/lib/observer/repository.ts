...
/**
 * Load every anonymous Observer response for a Subject, oldest first, reduced to
 * just the selected words — the only field the equal-weight aggregation (issue
 * #9) and the anonymous per-observer drill-down consume. Nothing here returns or
 * exposes an Observer's identity (there is none stored). Oldest-first ordering
 * gives the drill-down a stable "Observer 1/2/3" numbering across page loads.
 */
