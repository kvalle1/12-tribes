...
/**
 * Load every anonymous Observer response for a Subject, oldest first — the
 * inputs the equal-weight "others" aggregation and the comparison report consume
 * (issue #9). Only the selected `words` are returned; the rows carry no observer
 * identity (ADR-0003), and the deterministic `createdAt` ordering is what gives
 * the report its stable, attribute-free "Observer 1 / 2 / 3" labels.
 */
