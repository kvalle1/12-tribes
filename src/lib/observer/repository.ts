 ...
 * word lists — the only thing an `observer_response` carries (ADR-0003). The
 * stable order gives the report's per-observer drill-down consistent "Observer
 * 1/2/3" labels across visits, while still exposing nothing about who each
 * observer is. Feeds the equal-weight `aggregateObservers` aggregation (#9).
 */
