 ...
 * first, as bare word lists — the input the equal-weight aggregation (issue #9)
 * consumes. Ordering by `createdAt` gives the comparison report stable, neutral
 * "Observer 1 / 2 / 3" labels for its per-observer drill-down without ever
 * exposing who an Observer is (ADR-0003): only their words come back, never an
 * id, name, or timestamp.
 */
