 ...
 * word selections the equal-weight aggregation (issue #9) consumes. Only the
 * `words` are returned — never the row id or any identifying field — so the
 * per-observer drill-down can number them (Observer 1, 2, 3…) by position while
 * staying fully anonymous. Ordering by `createdAt` keeps that numbering stable
 * across page loads.
 */
