...
/**
 * Maps a tribe's Tailwind color name (the `color` field) to the accent hex used
 * for the row bar, the carved initial, and the result headline. A missing key
 * falls back to brass. Shared so the home page and the result view stay in sync
 * (the tribe detail page still has its own copy as a `Record`).
 */
