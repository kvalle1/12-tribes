...
/**
 * Maps a tribe's `color` (a Tailwind color name) to the accent hex used for the
 * per-tribe accent throughout the app — the home rows, the detail page, and the
 * assessment result view. Kept here, beside the data it describes, so there is a
 * single source of truth; a color with no entry falls back to brass.
 */
