 ...
 *
 * As of the real-scoring slice (#16): `profile` is the live Strength Profile
 * accumulated from cited Marker deltas, `trace` records each applied delta's
 * origin (answer → Marker → contribution) for result transparency, and
 * `currentQuestion` holds the question being shown (agent-produced after the
 * fixed opener) so a refresh resumes at the exact question. `userId` is optional
 * so anonymous sessions (resumed via an opaque cookie id) still work.
 */
/**
...
export const interviewSessions = pgTable("interview_session", {
  ...
  // Completed Turns (question + answer + cited deltas), oldest first.
  turns: jsonb("turns").$type<InterviewTurn[]>().notNull().default([]),
  turnCount: integer("turnCount").notNull().default(0),
  // Per-delta score trace (answer → Marker → contribution), oldest first.
  trace: jsonb("trace").$type<TraceEntry[]>().notNull().default([]),
  // The question currently being shown; null once the Session completes.
  currentQuestion: text("currentQuestion"),
  // The ranked 12-tribe Strength Profile, set once the flow completes.
