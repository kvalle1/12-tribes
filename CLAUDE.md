...
**Per-tribe accent color** comes from the `color` field (a Tailwind color name like `"amber"`), mapped to a hex by the `accentHex()` helper in `tribes.ts` — the single source of truth, called by the home page, the detail page, and the result view — then passed in as the `--accent` CSS variable via inline `style`. **When adding a tribe or color, add the matching key to the `ACCENT_HEX` map in `tribes.ts`** — a missing key falls back to brass with no error.
