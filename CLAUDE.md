...
**Per-tribe accent color** comes from the `color` field (a Tailwind color name like `"amber"`), mapped to a hex by the shared `accentHex` helper in `src/lib/accent.ts` (imported by `page.tsx`, the tribe detail page, and the assessment result view), then passed in as the `--accent` CSS variable via inline `style`. **When adding a tribe or color, add the matching key to the map in `src/lib/accent.ts`** — a missing key falls back to brass with no error.
