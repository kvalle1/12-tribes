...
**Per-tribe accent color** comes from the `color` field (a Tailwind color name like `"amber"`), mapped to a hex by an `accentHex` lookup duplicated in `page.tsx`, the detail page, and the result view (`src/components/result-view.tsx`), then passed in as the `--accent` CSS variable via inline `style`. **When adding a tribe or color, add the matching key to all `accentHex` maps** — a missing key falls back to brass with no error.
